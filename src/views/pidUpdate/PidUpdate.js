...
const PidUpdate = () => {
  ...
  const gettalltirecode = async () => {
    try {
      const add7 = await fetch(`http://localhost:5433/tirecode/gettirecodedetail`, {
        ...
        response.json().then((res) => {
          console.log(res.rowCount + 10128)

          setTirecode(res.rowCount + 10128)
  ...
  const deletetirecode = async (id) => {
    console.log('delete')
    try {
      const deletetire = await fetch(`http://localhost:5433/tirecode/deletetirecode/${id}`, {
        method: 'DELETE',
      })
  ...
  const gettirecodeforvalidate = async () => {
    try {
      const gettirecodebyvalidate = await fetch(
        `http://localhost:5433/tirecode/gettirecodedetailvalidatepid${pid}`,
        {},
      )
          ...
          <form onChange={handleSubmit(onSubmit)}>
            <input
              ...
              {...register('pid', {
                ...
                maxLength: { value: 6, message: 'Maximum allowed pid length is 6' },
                pettern: {
                  value: /^[0-9]*$/,
