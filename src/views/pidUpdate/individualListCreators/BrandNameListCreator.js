...
const BrandNameLstCreator = ({ clickeventfrmMain, selectedDetailState }) => {
  //---->
  /*
  ...
  */
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const info = await SLTLDBConnection.get(`/brandname/getallbrandnames`) //---->
        //for valied sn only
        if (info.data) {
  ...
  */
  useEffect(() => {
    if (selectedDetailState) {
      setselected(selectedDetailState.brandName) //---->
    }
  }, [selectedDetailState])
  ...
  useEffect(() => {
    if (data) {
      const x = data.map((value) => {
        const obj = {}
        obj.id = value.brandid //---->
        obj.value = value.brand //---->
        return obj
      })
      setRefinedArr(x)
    }
  ...
  return (
    <DropDwnComp
      ...
      dropdowncolour={'success'}
    />
  )
}
BrandNameLstCreator.propTypes = {
  //--->
  //---->
  clickeventfrmMain: PropTypes.func,
  selectedDetailState: PropTypes.object,
}
export default BrandNameLstCreator //---->
