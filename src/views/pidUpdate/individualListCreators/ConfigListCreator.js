...
const ConfigLstCreator = ({ clickeventfrmMain, selectedDetailState }) => {
  //---->
  /*
  ...
  */
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const info = await SLTLDBConnection.get(`/config/getallconfigs`) //---->
        //for valied sn only
        if (info.data) {
  ...
  */
  useEffect(() => {
    if (selectedDetailState) {
      setselected(selectedDetailState.config) //---->
    }
  }, [selectedDetailState])
  ...
  useEffect(() => {
    if (data) {
      const x = data.map((value) => {
        const obj = {}
        obj.id = value.configid //---->
        obj.value = value.config //---->
        return obj
      })
      setRefinedArr(x)
    }
  ...
  return (
    <DropDwnComp
      ...
      dropdowncolour={'danger'}
    />
  )
}
ConfigLstCreator.propTypes = {
  //--->
  //---->
  clickeventfrmMain: PropTypes.func,
  selectedDetailState: PropTypes.object,
}
export default ConfigLstCreator //---->
