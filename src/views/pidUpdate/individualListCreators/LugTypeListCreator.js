...
const LugTypeLstCreator = ({ clickeventfrmMain, selectedDetailState }) => {
  //---->
  /*
  ...
  */
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const info = await SLTLDBConnection.get(`/lugtype/getalllugtypes`) //---->
        //for valied sn only
        if (info.data) {
  ...
  */
  useEffect(() => {
    if (selectedDetailState) {
      setselected(selectedDetailState.lugType) //---->
    }
  }, [selectedDetailState])
  ...
  useEffect(() => {
    if (data) {
      const x = data.map((value) => {
        const obj = {}
        obj.id = value.lugtypeid //---->
        obj.value = value.lugtype //---->
        return obj
      })
      setRefinedArr(x)
    }
