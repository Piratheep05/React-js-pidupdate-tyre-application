...
const SwMsgLstCreator = ({ clickeventfrmMain, selectedDetailState }) => {
  /*
  ...
  */
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const info = await SLTLDBConnection.get(`/swmsg/getallswmsgs`)
        //for valied sn only
        if (info.data) {
  ...
  */
  useEffect(() => {
    if (selectedDetailState) {
      setselected(selectedDetailState.swMsg)
    }
  }, [selectedDetailState])
  ...
  useEffect(() => {
    if (data) {
      const x = data.map((value) => {
        const obj = {}
        obj.id = value.swmsgid
        obj.value = value.swmsg
        return obj
      })
      setRefinedArr(x)
    }
