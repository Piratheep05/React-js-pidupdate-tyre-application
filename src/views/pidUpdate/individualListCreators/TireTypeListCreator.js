...
const TireTypeLstCreator = ({ clickeventfrmMain, selectedDetailState }) => {
  //---->
  /*
  ...
  */
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const info = await SLTLDBConnection.get(`/tiretype/getalltiretypes`) //---->
        //for valied sn only
        if (info.data) {
  ...
  */
  useEffect(() => {
    if (selectedDetailState) {
      setselected(selectedDetailState.tireType) //---->
    }
  }, [selectedDetailState])
  ...
  useEffect(() => {
    if (data) {
      const x = data.map((value) => {
        const obj = {}
        obj.id = value.tiretypeid //---->
        obj.value = value.tiretype //---->
        return obj
      })
      setRefinedArr(x)
    }
  ...
  return (
    <DropDwnComp
      arr={refinedArr}
      selected={selected}
      dropDownName={'tireType'} //---->
      clickevent={clickevent}
      dropdowncolour={'warning'}
    />
  )
}
TireTypeLstCreator.propTypes = {
  //--->
  //---->
  clickeventfrmMain: PropTypes.func,
  selectedDetailState: PropTypes.object,
}
export default TireTypeLstCreator //---->
