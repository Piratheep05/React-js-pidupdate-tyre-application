...
const WheelColorLstCreator = ({ clickeventfrmMain, selectedDetailState }) => {
  //---->
  /*
  ...
  */
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const info = await SLTLDBConnection.get(`/wheelcolor/getallwheelcolors`) //---->
        //for valied sn only
        if (info.data) {
  ...
  */
  useEffect(() => {
    if (selectedDetailState) {
      setselected(selectedDetailState.wheelColor) //---->
    }
  }, [selectedDetailState])
  ...
  useEffect(() => {
    if (data) {
      const x = data.map((value) => {
        const obj = {}
        obj.id = value.wheelcolorid //---->
        obj.value = value.color //---->
        return obj
      })
      setRefinedArr(x)
    }
  ...
  return (
    <DropDwnComp
      arr={refinedArr}
      selected={selected}
      dropDownName={'wheelColor'} //---->
      clickevent={clickevent}
      dropdowncolour={'secondary'}
    />
  )
}
WheelColorLstCreator.propTypes = {
  //--->
  //---->
  clickeventfrmMain: PropTypes.func,
  selectedDetailState: PropTypes.object,
}
export default WheelColorLstCreator //---->
